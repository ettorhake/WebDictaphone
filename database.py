...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS recordings
                 (id INTEGER PRIMARY KEY, filename TEXT, timestamp TEXT, client_ip TEXT)''')
    conn.commit()
    conn.close()
...
def get_recordings():
    conn = sqlite3.connect('database.db')
    c = conn.cursor()
    c.execute("SELECT id, filename, timestamp, client_ip FROM recordings")
    recordings = c.fetchall()
    conn.close()
    return [{'id': row[0], 'filename': row[1], 'timestamp': row[2], 'client_ip': row[3]} for row in recordings]
